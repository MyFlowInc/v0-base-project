/**
 * Flow API Client - Server-side only
...
const getConfig = () => {
  ...
  const token = process.env.FLOW_APP_TOKEN;

  if (!baseUrl) {
    throw new Error('FLOW_API_URL environment variable is not set');
  }
  if (!token) {
    throw new Error('FLOW_APP_TOKEN environment variable is not set');
  }
...
async function request<T>(endpoint: string, options: RequestOptions = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Flow API Error [${response.status}]: ${error.message || 'Unknown error'}`);
  }
...
/**
 * Notify Flow after uploading to storage
 */
